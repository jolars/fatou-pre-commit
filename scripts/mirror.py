#!/usr/bin/env python3
"""Mirror new fatou releases from PyPI as commits and tags.

For every fatou version on PyPI newer than the one pinned in
...
def current_version() -> str:
    match = re.search(r'"fatou==([^"]+)"', PYPROJECT.read_text())
    assert match, "no fatou pin in pyproject.toml"
    return match.group(1)


def pypi_versions() -> list[str]:
    with urllib.request.urlopen("https://pypi.org/pypi/fatou/json") as response:
